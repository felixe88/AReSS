...
const Chart1 = () => {
    ...
    const [selectedAnni, setSelectedAnni] = useState([]);
    ...
    const handleClearAll = () => {
        setCheckboxes((prevCheckboxes) => {
            const updatedCheckboxes = prevCheckboxes.map((checkbox) => ({ ...checkbox, checked: false }));
            setSelectedAnni([]);
    ...
    const handleSelectAll = () => {
        setCheckboxes((prevCheckboxes) => {
            ...
            setSelectedAnni(updatedCheckboxes.filter((checkbox) => checkbox.checked).map((checkbox) => checkbox.label));
            return updatedCheckboxes
        }
        );
    };
    ...
    const handleSelectAllEta = () => {
        setEtaOptions((prevEtaOptions) => {
            ...
            setSelectedEta(updateEta.filter((checkbox) => checkbox.checked).map((checkbox) => checkbox.label));
            return updateEta
        })
    }
    ...
    const handleClearAllEta = () => {
        setEtaOptions((prevEtaOptions) => {
            const updateEta = prevEtaOptions.map((option) => ({ ...option, checked: false }));
            setSelectedEta([]);
            return updateEta;
        })
    }
                ...
                <div className="flex xs:flex-col sm:flex-row items-center space-x-3 xs:pl-2 md:pl-4 lg:pl-16 w-auto justify-between ">
                    <div className="flex sm:flex-row items-center space-x-3 xs:pl-2 sm:pl-4 w-auto ">
                            ...
                            >
                                {selectedAnni.length > 0
                                    ? selectedAnni.length > 3
                                        ? <p className="xs:text-xs md:text-base"> {selectedAnni.length} selezionati</p>
                                        : selectedAnni.join(", ")
                                    : "Anni"}
                            </button>
                            {isAnnoVisible && (
                                <div className="w-36 h-40 overflow-auto flex flex-col  bg-white border border-black absolute z-10 rounded-md">
                                    <input
                                        ...
                                        className="w-28 m-1"
                                    ></input>
                                    <button className="border hover:bg-gray-400 hover:text-white rounded-lg m-1" onClick={handleClearAll}>Pulisci</button>
                                    <button onClick={handleSelectAll}>Seleziona tutti</button>
                                    {filteredCheckboxes.map((checkbox) => (
                                        <div className={`flex w-auto hover:bg-gray-400 pl-1 pr-1 ${checkbox.checked ? 'bg-slate-300' : ''} `} key={checkbox.id}>
                                            <input
                                                type="checkbox"
                                            ...
                                            />
                                            <label className="ml-3" htmlFor={`checkbox-${checkbox.id}`}>
                                                {checkbox.label}
                                            </label>
                                        </div>
                                    ))}
                        ...
                        <div className="xs:w-20  md:w-28 lg:w-32">
                            ...
                            >
                                <p className={selectedSesso === "Maschi e Femmine" && "xs:text-xs lg:text-sm"}>{selectedSesso}</p>
                            </button>
                            {isSessoOpen && (
                                <div className="pr-32">
                            ...
                            >
                                {selectedEta.length > 0
                                    ? selectedEta.length > 3
                                        ? <p className="xs:text-xs md:text-base">{selectedEta.length} selezionati</p>
                                        : selectedEta.join(", ")
                                    : "Età"}
                            </button>
                            {isEtaVisible && (
                                <div className="w-36 h-40 overflow-auto flex flex-col items-center bg-white border border-black absolute z-10 rounded-md">
                                    <input
                                        ...
                                        className="w-28 m-1"
                                    />
                                    <button className="w-28 border hover:bg-gray-400 hover:text-white rounded-lg m-1" onClick={handleClearAllEta}>Pulisci</button>
                                    <button onClick={handleSelectAllEta}>Seleziona tutti</button>
                                    {filteredEtaOptions.map((option) => (
                                        <div className={` w-full flex hover:bg-gray-400 pl-1 pr-1 ${option.checked ? 'bg-slate-300' : ''} `} key={option.id}>
                                            <input
                                                type="checkbox"
                                            ...
                                            />
                                            <label className="ml-3" htmlFor={`checkbox-eta-${option.id}`}>
                                                {option.label}
                                            </label>
                                        </div>
                                    ))}
                    ...
                    <div className="flex xs:mt-2 sm:mt-0 flex-row items-center space-x-3  w-auto ">
                        <div className="flex space-x-3 xs:pr-1 md:pr-0 lg:pr-8 ">
                            <button className="xs:h-7 md:h-5 xs:w-16 md:w-24 bg-gray-200 border border-black flex items-center justify-center" onClick={ripristina}>Ripristina</button>
                            <button className="xs:h-7 md:h-5 xs:w-16 md:w-24 bg-gray-200 border border-black flex items-center justify-center">Esporta</button>
                        </div>
                    </div>
                </div>
                {/* CHART */}
